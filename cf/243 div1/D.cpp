...
const int N = int(2.5e5) + 9;
PII P[N]; int PP[N];
int n, nn; int Q; LL ans;

inline bool cmp(int a, int b){
    return P[a].se < P[b].se || P[a].se == P[b].se && P[a].fi < P[b].fi;
}
...
int main(){
    ...
    Q = sqrt(RD(n)); REP(i, n) RDD(P[i].fi, P[i].se); P[n].fi = P[n].se = INF; sort(P, P+n);

    for (int i=0,ii;i<n;i=ii){

        ii = i+1; while (P[i].fi==P[ii].fi)++ii;

        if (ii-i>Q){
            set<int> H; int s=0; FOR(j, i, ii) H.insert(P[j].se);

            for(int j=ii;j<n;++j){
                if (CTN(H, P[j].se)) ++s;
                if (P[j].fi!=P[j+1].fi) ans += C2(s), s = 0;
            }
            for(int j=0;j<nn;++j){
                if (CTN(H, P[j].se)) ++s;
                if (P[PP[j]].fi!=P[PP[j+1]].fi) ans += C2(s), s = 0;
            }
        }
        else{
            FOR(j, i, ii) PP[nn++] = j; PP[nn] = n;
        }
    }

    sort(PP, PP+nn, cmp);

    for (int i=0,ii;i<nn;i=ii){
        ii = i+1; while(P[PP[i]].se==P[PP[ii]].se)++ii;
        map<int, int> H; FOR(j, i, ii){
            int t = P[PP[j]].fi;
            for(int jj=PP[j]-1;jj>=0&&P[jj].fi==t;--jj) ++H[P[jj].se];
        }
        ECH(it, H) ans += C2(it->se);
    }
